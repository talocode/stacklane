...
program
  ...
  .action(async (opts) => {
    ...
    if (!config.databaseUrl) {
      console.log('✗ No database URL configured. Run: stacklane db set');
      process.exit(1);
    }
    ...
    try {
      const { testDatabaseConnection, extractHost } = await import('@stacklane/core');
      const result = await testDatabaseConnection(config.databaseUrl, config.databaseProvider || 'postgres');

      if (opts.json) {
        console.log(JSON.stringify(result, null, 2));
      } else {
        if (result.ok) {
          console.log(`✓ Connected to ${result.safeHost || 'database'} in ${result.latencyMs}ms`);
        } else {
...
program
  ...
  .option('--confirm', 'Write actual values (requires explicit flag)')
  .action((opts) => {
    const config = loadConfig();
    ...
    const lines = [
      ...
      `STACKLANE_DATABASE_URL=${safe ? '<YOUR_DATABASE_URL>' : (config.databaseUrl || '')}`,
      `STACKLANE_DATABASE_PASSWORD=${safe ? '<YOUR_DATABASE_PASSWORD>' : (config.databasePassword || '')}`,
      `STACKLANE_ACCESS_TOKEN=${safe ? '<YOUR_ACCESS_TOKEN>' : (config.accessToken || '')}`,
      '',
    ...
    if (safe) {
      console.log(`  Safe mode: placeholders written. Use --confirm to write actual values.`);
    } else {
      console.log(`  ⚠ Actual values written. Do not commit .env.stacklane.`);
    }
    console.log(`  ⚠ Add ".env.stacklane" to .gitignore.`);

    const gitignore = '.env.stacklane';
    ...
    if (fs.existsSync(giPath)) {
      ...
      if (!content.includes(gitignore)) {
        ...
        console.log(`  ✓ Added .env.stacklane to .gitignore`);
      }
    } else {
      fs.writeFileSync(giPath, `${gitignore}\n`);
      console.log(`  ✓ Created .gitignore with .env.stacklane`);
    }
  });
