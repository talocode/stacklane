...
export type DataType = 'email' | 'phone' | 'ip'
...
const DATA_EMAIL_TYPOS: Array<{ id: string; re: RegExp; correct: string }> = [
...
const DATA_COUNTRY_CODES: DataCountryCode[] = [
...
function dataCountryForCode(dial: string): DataCountryCode | undefined {
...
function dataCleanPhone(raw: string): string {
  return raw.replace(/[\s\-().]/g, '').replace(/^00/, '+')
}

export function verifyEmail(input: string): DataValidationResult {
  const started = Date.now()
...
export function verifyPhone(input: string, options: { country?: string } = {}): DataValidationResult {
  const started = Date.now()
  ...
  if (!value) {
    ...
    if (!/^\+?\d{6,15}$/.test(cleaned)) {
      ...
      if (options.country) {
        const target = DATA_COUNTRY_CODES.find((c) => c.name.includes(options.country as string))
        if (target) {
...
function dataClassifyIpv4(octets: number[]): string {
  const [a, b] = octets
...
function dataClassifyIpv6(groups: string[]): string {
...
export function verifyIp(input: string): DataValidationResult {
  const started = Date.now()
  const value = (input ?? '').trim()
  const findings: DataFinding[] = []
  let normalized: string | null = null
  ...
  if (!value) {
    ...
    if (dotCount === 3 && colonCount === 0) {
      ...
      if (octets.every((o) => o >= 0 && o <= 255)) {
        ...
        meta.version = 4
        meta.classification = dataClassifyIpv4(octets as number[])
        meta.isPrivate = ['private', 'loopback', 'link-local', 'carrier-grade-nat'].includes(meta.classification as string)
      ...
      if (validGroups) {
        ...
        meta.isPrivate = ['loopback', 'unique-local', 'link-local'].includes(meta.classification as string)
...
export function verifyData(input: { values?: DataCheck[] }): {
  ok: boolean
  product: 'verifylane'
  version: string
  mode: string
  ...
  const results: DataValidationResult[] = values.map((v) => {
