/**
 * LLM Gateway (hosted) — OpenAI-compatible proxy under /v1/gateway/*.
 * Proxies chat completions to a configured upstream provider, meters each
 * call, and returns usage + cost summaries. Self-hostable core: @talocode/llmgateway.
 */
...
export interface GatewayConfig {
  ...
  defaultPricing: { inputPerM: number; outputPerM: number }
}
...
export function resolveGatewayRoute(config: GatewayConfig, model: string) {
...
export function estimateGatewayCost(pricing: { inputPerM: number; outputPerM: number }, prompt: number, completion: number) {
...
const inMemoryUsage: GatewayUsageEvent[] = []

export function recordGatewayUsage(event: Omit<GatewayUsageEvent, 'id' | 'createdAt'>) {
  const full: GatewayUsageEvent = {
    ...event,
...
export function getGatewayUsage(projectId?: string) {
