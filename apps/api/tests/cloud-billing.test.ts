...
import { isLemonSqueezyConfigured } from '../src/services/payments/lemon-squeezy-provider'
...
// ─── Lemon Squeezy Top-up Tests ────────────────────────────────────────────

test('fixed credit-pack variants configure Lemon Squeezy without a default variant', () => {
  const original = {
    apiKey: process.env.LEMONSQUEEZY_API_KEY,
    storeId: process.env.LEMONSQUEEZY_STORE_ID,
    variantId: process.env.LEMONSQUEEZY_VARIANT_ID,
    variantMap: process.env.LEMONSQUEEZY_VARIANT_MAP,
  }
  process.env.LEMONSQUEEZY_API_KEY = 'test-key'
  process.env.LEMONSQUEEZY_STORE_ID = '123'
  delete process.env.LEMONSQUEEZY_VARIANT_ID
  process.env.LEMONSQUEEZY_VARIANT_MAP = '{"500":"456"}'

  assert.equal(isLemonSqueezyConfigured(), true)
